...
#include <stdio.h>
#ifdef LINUX
    #include <unistd.h> // added for getcwd, getcd
...
int		indent = 0,		/* current indent */
		...
		dupi = FALSE,		/* use duplicate inodes */
	...
	if (sort) {
		...
		while (tmp_RD) {
			...
			while (tmp1_RD) {
                ...
                if (strcmp(NAME(tmp_RD->entry),NAME(tmp1_RD->entry))>0) { // added
					/* swap the two */
...
/* print totals */
	...
	tmp_RD = head;
    struct RD_list *next_RD = NULL; // added
	while (tmp_RD) {
        next_RD = tmp_RD->fptr;
		free(tmp_RD);
		//tmp_RD = tmp_RD->fptr;
...
int		i;
	...
	if (cont) {
		if (is_directory(path)) {
            inodes++; // added
            sizes+= ((stb.st_blocks)/2); // added
            // st_blocks is in 512 byte format, du returns 1024 byte format
			down(path);
        }
	}
	else {
		...
		if ( (h_enter(stb.st_dev, stb.st_ino) == OLD) && (!dupi) )
			return;
		inodes++;
		//sizes+= K(stb.st_size);
        sizes+= ((stb.st_blocks)/2); // added
...
int	user_file_list_supplied = 0;
...
#ifdef LINUX
    int option_index = 0; // added
    static struct option long_options[] = { // added
    ...
    while ((option = getopt(argc, argv, "dfh:iostqvVl")) != EOF) {
#endif
        // existance of "l" option will be reported in the Usage messages
        // if program was not compiled with LSTAT defined, option should be illegal
        ...
        switch (option) {
			...
			case 'V':	version++;
					break;
            #ifdef LSTAT //added
                case 'l':   sw_follow_links = 0; break;
            #endif
			default:	err = TRUE;
		}
        if (err) {
                ...
                #ifdef LSTAT
                    fprintf(stderr,"    -l  cause program not to follow symbolic links\n");
                #elif MEMORY_BASED
	...
	if (version > 0 ) {
		...
		if (version>1) {
			...
			if (dupi) printf("Include duplicate inodes\n");
